...
async function loadPartners() {
  ...
  if (me.role !== "partner" && me.role !== "admin") {
    document.getElementById("app").innerHTML = `
      <div class="page-partner">
        <div class="loader-box" style="padding-top:60px;">
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const cls = classes[i];
    if (!p) {
      podiumHtml += '<div class="podium-card ' + cls + '">' +
  ...
  if (myPartner) {
    gridHtml += '<div class="partner-card me">' +
  ...
  partners.slice(3).forEach(p => {
    gridHtml += '<div class="partner-card">' +
  ...
  if (!gridHtml && partners.length <= 3) {
    // top3 already shown; if no rest and no me card extra, ok
  }
  ...
  document.getElementById("app").innerHTML =
    '<div class="page-partner">' +
      ...
      '<div class="stats">' +
        '<div class="stat"><div class="stat-value">' + partners.length + '</div><div class="stat-label">পার্টনার</div></div>' +
        '<div class="stat"><div class="stat-value">' + totalReferralEarnings.toLocaleString() + '</div><div class="stat-label">মোট রেফার আয়</div></div>' +
        '<div class="stat"><div class="stat-value">' + pool.toLocaleString() + '</div><div class="stat-label">পুল</div></div>' +
      '</div>' +
      ...
      '<div class="partner-grid">' + gridHtml + '</div>' +
...
loadPartners().catch(err => {
  console.error(err);
  document.getElementById("app").innerHTML =
    '<div class="loader-box"><h2>সমস্যা</h2><p class="error">' + err.message + '</p></div>';
});
